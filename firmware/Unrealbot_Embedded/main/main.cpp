...
// (IMU and Flow Sensor removed per user configuration)
...
void RobotLoopTask(void *parameters) {
    ...
    const TickType_t xFrequency = 1; // 1ms precise interval
...
void SerialCommandTask(void *pvParameters) {
    ...
    while (1) {
        ...
        if (len == 1) {
            ...
            vTaskDelay(1); // PREVENT WATCHDOG PANIC!
        }
    }
}
...
extern "C" void app_main(void)
{
    ...
    // Install USB Serial JTAG driver for COBS telemetry / command stream
