...
extern void imu_spi_write(uint8_t reg, const uint8_t * wbuffer, uint32_t wlen);
extern void imu_spi_read(uint8_t reg, uint8_t * rbuffer, uint32_t rlen);

static int spi_write(uint8_t reg, const uint8_t * wbuffer, uint32_t wlen) {
  imu_spi_write(reg, wbuffer, wlen);
  return 0;
}

static int spi_read(uint8_t reg, uint8_t * rbuffer, uint32_t rlen) {
  imu_spi_read(reg, rbuffer, rlen);
  return 0;
}
