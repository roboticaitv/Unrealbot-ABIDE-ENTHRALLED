...
class DualCamera:
        ...
        for cam_idx in [0, 1]:
            try:
                ...
                if Transform is not None:
                    # Apply 180-degree rotation (both hflip and vflip) to orient the camera correctly
    ...
    def _capture_loop_0(self):
        while self.running:
            ...
            cropped = self._crop_stride(frame)
            with self.lock0:
                self.frame0 = cropped
    ...
    def start(self):
        if Picamera2 is not None:
            if self.cam0 and hasattr(self.cam0, 'start'): self.cam0.start()
            if self.cam1 and hasattr(self.cam1, 'start'): self.cam1.start()
    ...
    def _crop_stride(self, frame):
        ...
        if stride <= req_w:
            return frame.copy()  # Make a copy since the camera writes to the original buffer
    ...
    def get_frames(self):
        ...
        # Fallback for single camera setup
        if f0 is None and f1 is not None:
            f0 = f1
        elif f1 is None and f0 is not None:
            f1 = f0
