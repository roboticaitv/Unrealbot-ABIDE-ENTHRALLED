...
def draw_detections(bgr_frame, detections, aruco_positions, threats, state, action=None, cam_label="VIDEO", latency_ms=0, show_hud=True):
    ...
    if detections.get("ball"):
        ...
        dist_meters = state.get('ball_distance_norm', 0) * 3.0
        hud_lines.append(f"Ball Dist: {dist_meters:.2f}m")
        hud_lines.append(f"Ball Ang:  {state.get('ball_angle_deg', 0):.1f} deg")
        hud_lines.append(f"Ball Spd:  {state.get('ball_speed_norm', 0):.2f}")
    ...
    if detections.get("blue_goal"):
        ...
        dist_m = state.get("blue_goal_distance_mm", 0) / 1000.0
        ang_deg = state.get("blue_goal_angle_deg", 0)
        hud_lines.append(f"Blue Goal: {dist_m:.1f}m @ {ang_deg:.1f} deg")
    else:
    ...
    if detections.get("yellow_goal"):
        ...
        dist_m = state.get("yellow_goal_distance_mm", 0) / 1000.0
        ang_deg = state.get("yellow_goal_angle_deg", 0)
        hud_lines.append(f"Yell Goal: {dist_m:.1f}m @ {ang_deg:.1f} deg")
    else:
    ...
    # ── Enemies (Threats) ──
    if len(threats) > 0:
        enemy_meters = state.get('enemy1_distance_norm', 0) * 3.0
        hud_lines.append(f"E1 Dist: {enemy_meters:.2f}m")
    ...
    enemy_cands = detections.get("unknowns", [])
    if enemy_cands:
    ...
    if show_hud:
        ...
        # Optimize rendering by only copying the HUD panel region
        roi = bgr_frame[0:panel_h, :]
        roi_copy = roi.copy()
        cv2.rectangle(roi_copy, (0, 0), (bgr_frame.shape[1], panel_h), (0, 0, 0), -1)
        cv2.addWeighted(roi_copy, 0.6, roi, 0.4, 0, roi)
